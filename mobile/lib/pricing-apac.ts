// APAC currencies for Stripe Checkout (mirrors the web src/lib/pricing-apac.ts).
// Credit conversion + Stripe unit amounts are computed server-side; the app only
// needs the display metadata + presets and to pass the currency code to Stripe.
